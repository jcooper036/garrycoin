Garrycoin Core version *0.15.1* is now available from:

  <https://download.garrycoin.org/garrycoin-0.15.1/>
...
A number of changes to the way Garrycoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 bwwwaaas per kB (0.00001 BWA). This is to prepare for dropping the minimum transaction fee to 0.1 bwwwaaas per kB (0.0001 BWA) in the next release.
