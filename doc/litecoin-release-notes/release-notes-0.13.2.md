Garrycoin Core version 0.13.2 is now available from:

  <https://download.garrycoin.org/garrycoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/garrycoin-project/garrycoin/issues>
...
updates will be released anymore. Without security updates, using a garrycoin
...
ECDSA signatures inside Garrycoin transactions now use validation using
...
Previous versions of Garrycoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Garrycoin Core was to raise the default minimum
relay fee.

Garrycoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Garrycoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running garrycoind without having to do any manual configuration.
...
Garrycoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Garrycoin Core will relay transactions with
...
entirely in Garrycoin Core 0.13, and it is at this time undecided whether the
...
Garrycoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Garrycoin Core automatically creates a hidden service to listen on, without
manual configuration. Garrycoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Garrycoin Core is listening, and
...
Garrycoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Garrycoin
Core to use floating fees. By default, Garrycoin Core will use floating
fees.
...
(default: `0.0002` BWA/kB).

At all times, Garrycoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BWA.
Furthermore, Garrycoin Core will never create transactions paying less than
...
`garrycoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /garrycoinrpc>
        ...
        # optional bypass garrycoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< garrycoinrpc:<password>
    </Location>
...
Restarting garrycoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in garrycoin.conf
...
garrycoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/garrycoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Garrycoin Core code base has
...
- `garrycoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `garrycoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Garrycoin scripting system that in combination with
...
Garrycoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Garrycoin Core.
...
The two phases are distinct in the Garrycoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Garrycoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Garrycoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Garrycoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of garrycoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of garrycoins being spent is exactly
...
- **Increased security for multisig:** Garrycoin addresses (both P2PKH addresses
  ...
  believed to be provided by Garrycoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Garrycoin's current ledger (called the UTXO set),
  ...
  note that Garrycoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Garrycoin users to individually opt-in to almost any change in the Garrycoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Garrycoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Garrycoin
client to Garrycoin Core 0.13.2 to signal for segwit support. When a
...
Since Garrycoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `garrycoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `garrycoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
