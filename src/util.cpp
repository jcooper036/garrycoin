...
const char * const BITCOIN_CONF_FILENAME = "garrycoin.conf";
const char * const BITCOIN_PID_FILENAME = "garrycoin.pid";
...
#else
    const char* pszModule = "garrycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "garrycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/garrycoin";
#else
    // Unix
    return pathRet / ".garrycoin";
#endif
#endif
}
